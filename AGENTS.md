...
Use only synthetic identifiers. Never commit private captures, credentials, real VINs, security payload logs or local signing values. Reviewed synthetic screenshots under docs/images are allowed for public documentation. Preserve upstream MIT notices.
...
Use typed branches and conventional commits. Push only when explicitly authorized by the user. Preserve history; never force-push. The canonical remote is git@github.com:fenrapp/fenr-vcu-emulator.git.
