...
Use typed topic branches and small conventional commits. Run scripts/check.sh and inspect changed UI before committing code; review git diff --cached. Integrate with fast-forward merge, then delete merged branches. Do not push, rewrite history or create acceptance tags while required physical checks are pending.
